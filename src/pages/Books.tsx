...
export default function Home() {
    ...
    const fetchBooks = useCallback(async () => {
        ...
        try {
            ...
            setState(prev => ({ ...prev, books: response.items, totalPages: response.totalPages || 1, loading: false }));
        } catch {
    ...
    const handleSortChange = (e: ChangeEvent<HTMLSelectElement>) => setState(prev => ({ ...prev, sort: e.target.value, page: 1 }));
        ...
        <div className="container mx-auto px-4 py-8">
            <section className="mb-8">
                <div className="flex flex-col md:flex-row gap-4 mb-6">
                    <div className="relative flex-grow">
                        ...
                        <Input type="text" placeholder="Search books..." onChange={handleSearchChange} className="pl-10" />
                    </div>
                    <select value={state.sort} onChange={handleSortChange} className="border rounded-md p-2">
            ...
            {state.error && (
                <div className="text-center text-red-500 mb-6">
                    {state.error}
                    <Button variant="outline" onClick={fetchBooks} className="ml-4">Retry</Button>
                </div>
            )}
            ...
            {!state.loading && !state.error && (
                <section className="mb-12">
                    ...
                    {state.books.length === 0 && <p className="text-center text-gray-500">No books found.</p>}
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        {state.books.map(book => (
                            <Card key={book.id} className="h-full flex flex-col hover:shadow-lg transition-shadow">
                                <CardHeader className="p-0">
                                    <img src={`${OpenAPI.BASE}/${book.coverImage}`} alt={book.title} className="h-48 w-full object-cover rounded-t-md" />
                                </CardHeader>
                                <CardContent className="flex-grow p-4 space-y-1">
                                    <CardTitle className="text-lg font-semibold">{book.title}</CardTitle>
                                    <CardDescription className="text-sm text-gray-500">by {book.bookOwnerName}</CardDescription>
                                    <p className="text-sm text-gray-700">Genre: {book.genre}</p>
                                    <p className="text-sm text-gray-700">Language: {book.language}</p>
                                    <p className="text-sm text-gray-700">Price: <span className="font-medium">${book.borrowPrice}</span></p>
                                    <p className="text-xs text-gray-500">Available until: {new Date(book.availableTo).toLocaleDateString()}</p>
                                </CardContent>
